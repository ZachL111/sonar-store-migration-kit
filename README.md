...
sonar-store-migration-kit is a Swift project for databases. It focuses on this technical goal: Develop a Swift command-oriented project for migration scenarios with framed sample traffic, bounds and ordering tests, and synthetic fixtures only.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 152, risk penalty 6, latency penalty 2, and weight bonus 3. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
